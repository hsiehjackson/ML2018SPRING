...
class DataManager:
		...
		with open(data_path,'r') as f:
			for line in f:
				if with_label:
					...
					sentence.append(lines[1].split(' '))
				else:
					X.append(line)
					sentence.append(line.split(' '))

		if with_label:
			self.data[name] = [X,Y,sentence]
		else:
			self.data[name] = [X,sentence]
	# Build dictionary
	...
	def get_semi_data(self,data,label,threshold,loss_function): 
	...
	# get data by name (train or test)
	def get_data(self):
		return self.data
